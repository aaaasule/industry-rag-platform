...
- **扫描件 OCR 链路**（实测半数真实文档为纯扫描件，见 R1a，原计划放在 M3 已不可行），解析任务按页并行调度
- **条款式分块**（`clause_mode`）：标准/规程类文档的结构单位是行首条款号而非标题行，见 04 文档 3.2
- **字符规范化**：错误映射还原 + NFKC，索引侧与查询侧共用一份实现（见 R1b）
- 页眉页脚剔除（行聚类法）、标题层级识别
- 结构感知分块器 + 标题路径前缀
...
**验收**：上传一份 100 页的真实设备手册，5 分钟内状态变为 `ready`，抽查 10 个 chunk 的 `heading_path` 与 `page_start` 均正确；`chunk.content` 中的标准号与型号全部为半角 ASCII。

**风险点**：解析质量是本阶段的主要不确定性，必须用真实客户文档测试，不要用规整的公开 PDF 自欺欺人。开工前用 `spikes/parse-quality` 对本批文档跑一遍体检，把 fail 项转成本阶段的验收用例。
...
**验收**：随机抽 20 条引用，点击后跳转页码正确且高亮区域覆盖引用文本；OCR 文档的引用高亮位置同样准确（OCR 坐标的可靠性与文本层不同，必须单独验）。
...
| R1a | **国标类文档大量为纯扫描件**（实测 6 份 AQ 标准中 3 份零文本层） | 已发生 | 高 | OCR 从"边缘路径"提升为 M1 必做项，工期与 `worker-parse` 规格需按此重估；OCR 输出的坐标质量要单独验证 |
| R1b | **PDF 字体错误映射导致拉丁字母变汉字**（实测有文本层的 3 份全部中招） | 已发生 | 高 | 04 文档 2.3 已加入字符规范化步骤；`normalize()` 必须索引侧与查询侧共用一份实现 |
| R1c | **OCR 单页 6.3 秒，100 页扫描件串行需 10 分钟**，超出性能目标 | 已发生 | 中 | 解析阶段改为页级并行调度（01 文档 5.1）；换引擎无效，这是 CPU 上的正常水平 |
| R1d | **扫描件水印显著拉低 OCR 置信度**，35.7% 文本框低于 0.8 | 已发生 | 中 | 低置信度文本照常入库但标注 `ocr_confidence`，在引用卡片上提示核对原文；置信度阈值按行业标定，不照搬通用经验值 |
