...
**解析阶段的调度单位是页，不是文档。** 实测 OCR 单页耗时中位 6.3 秒，一份 100 页扫描件串行处理需要约 10 分钟，无法满足上面的性能目标，而换引擎并不能显著改善。解法是把解析拆成 N 个页级子任务并行执行，文档级状态由页级结果聚合推导。这也顺带解决了重试粒度问题——单页 OCR 失败只重跑那一页。
...
| 检索延迟（不含生成） | < 300 ms |
| 问答首 token 延迟 | < 2.5 s |
| 100 页 PDF 端到端摄取 | < 90 s（无 OCR）/ < 5 min（需 OCR，**依赖页级并行**） |
| 控制台页面首屏 | < 1.5 s |
