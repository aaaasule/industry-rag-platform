...
| --- | --- |
| 日期 | 2026-08-26 |
| 范围 | 清单 A–F（环境、摄取、检索/对话、Profile 边角、golden 评测） |
| 摄取问题 | 部分 PDF 抽取含 `\u0000`，Postgres 拒绝写入 `document_pages` → 已在 `normalize` / 入库前剔除 |
| 后续 | 不自动开 M6；chord 页级并行、LLM 查询改写等仍为可选后置 |
...
见上文「2026-08-26（真实行业语料 E2E 验收）」：结论 **通过**；修复 PDF `\u0000` 入库失败。
