...
解析产出统一的中间表示后，做三件清理工作：

**页眉页脚剔除**：统计所有页面顶部/底部 8% 区域内的文本，出现频次超过页数 60% 的行判为页眉页脚移除。这比按固定坐标裁剪稳健，能自适应不同排版。需保留页码信息用于溯源，但不进入 chunk 文本。

**标题层级识别**：优先用 PDF 大纲（outline/TOC）；无大纲时按"字号 + 加粗 + 编号模式（`^\d+(\.\d+)*\s`）"综合打分。工业文档的章节编号非常规整，编号模式的权重应高于字号。
...
- **流程工业**（工艺规程、HAZOP）：条款密集且相互独立，`max_tokens` 降到 384，`overlap` 降到 32，避免把不相干的条款塞进同一块。
...
| 型号/编号类查询召回错 | 看 `scores.fulltext` 是否为 0 | 分词把型号切碎；需加入行业术语表 |
