...
const CAPABILITIES = [
  { icon: MagnifyingGlass, text: '混合检索与可核验证据引用' },
  { icon: ShieldCheck, text: '行业模板驱动分块与提示词' },
  { icon: CheckCircle, text: '多租户隔离与用量可观测' },
];
...
export function LoginPage() {
  ...
  return (
    <div className="flex min-h-[100dvh] flex-col md:flex-row">
      <aside className="app-enter flex flex-col justify-between border-b border-line bg-elevated px-8 py-10 md:w-[44%] md:border-b-0 md:border-r md:px-12 md:py-16">
        <div>
          <div className="flex items-center gap-3">
            <BrandMark size={36} />
            <p className="text-sm font-semibold text-accent">工业知识库平台</p>
          </div>
...
export function FullscreenHint({ text }: { text: string }) {
  return (
    <div className="workbench flex min-h-[100dvh] items-center justify-center text-sm text-ink-muted">
      {text}
    </div>
  );
}
